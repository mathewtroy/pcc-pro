 ...
 */
int RabinKarp::calculateHash(const char str[], int len, int q) {
    int hash_value = 0;
    for (int i = 0; i < len; i++) {
 ...
 */
bool RabinKarp::compareSubstring(const char pattern[], const char txt[], int start_index, int len_pat) {
    int j;
    for (j = 0; j < len_pat; j++) {
 ...
 */
int RabinKarp::updateHash(int hash_txt, const char txt[], int i, int len_pat, int h, int q) {
 ...
 */
std::vector<int> RabinKarp::searchRabinKarp(const char pattern[], const char txt[], int q) {
 ...
 * @param q The prime number used for hashing in Rabin-Karp algorithm
 */
void RabinKarp::searchInFileRK(const std::string& filename, const std::string& pattern, int q) {
// Open the file
